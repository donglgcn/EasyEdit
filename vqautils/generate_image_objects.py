...
def load_questions_and_rephrase(vqa):
    ...
    for i, ann in tqdm(enumerate(anns)):
        if ann.get('image_object', None) is not None:
            continue
        flag=False
        ...
        ann["image_object"] = image_object
        rephrased_anns_list.append(ann)

        print(f"pred_image_object: {image_object}\n")
        if (i+1) % save_every == 0:
            with open(f'output_{index}.json', 'w') as outfile:
                json.dump({"pred_image_object": rephrased_anns_list[i-save_every+1:i+1]}, outfile, indent=4)
            index += 1
            flag = True
    if not flag:
        with open(f'output_{index}.json', 'w') as outfile:
            json.dump({"pred_image_object": rephrased_anns_list[index * save_every:]}, outfile, indent=4)
...
with open('image_object_train.json', 'w') as outfile:
    json.dump({"pred_image_object": rephrased_questions}, outfile, indent=4)
